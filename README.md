...
- **Chit-chat + read-only code Q&A**: greeting/status/gratitude/farewell/identity all give varied canned replies (not the same string every time) with no LLM call. Replies are automatically enriched with live state (console port, projects indexed, any dev server running for the project + its URL, and an uncommitted-git-file count when known), and greetings also include what the console remembers about the project (`memory.md`) plus quick pointers to `help`/`overview`. A project can customize the canned pool per intent in its `console.config.json` (`chatReplies: { greeting: [...], status: [...], ... }`) — invalid/malformed entries are dropped at scan time rather than breaking the project. With AI mode on, greeting/status also ask the active selected model for a smarter tailored reply (bounded wall-clock timeout, falls back to the exact canned reply on any error); with AI mode off the answer is 100% deterministic. Read-only questions answerable without AI: detected API routes (Express/Flask/FastAPI/Django), which files import/are imported by a given file, monorepo sub-package detection, TODO/FIXME scanning, and largest-files-by-size.
...
- **"how do I run/do X"**: the `how_to_run` answer lists every run command the project's docs mention (site + server + services), numbered with source labels, plus every exact `console.config.json` command entry (with param names), deduplicated — so you get the full picture, not just the first documented command. When you ask to *run* something, the command chosen follows a trust order: config entries > package.json `npm run` scripts (the current source of truth — the package file is updated far more often than a README) > documented README/CLAUDE.md command > the project's own `Play *.bat` launcher > a language-based guess. Server-flavored asks ("run the server", "start the api", "run the backend") and site-flavored asks ("run the site", "start the dashboard") both prefer whichever documented command actually *serves* the web app, instead of landing on a one-shot command documented first.
