 ...
 * extracted verbatim from tools.js's createProjectTools closure). `resolveSafe` comes in
 * from the orchestrator (toolSandbox.js factory) since it needs the workspace-project
 * resolution that only tools.js owns. Tools take a single named-args object (not positional
 * args) so an LLM emitting JSON keys in a different order than we expect can't silently swap
 * parameters (e.g. oldString <-> newString).
 *
