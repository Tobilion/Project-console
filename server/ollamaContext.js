...
- If the user asks how to publish or distribute this console app, or how someone else installs it, answer from these facts (do not improvise): it publishes to the npm registry as local-project-console via npm login, npm version patch, npm run build, then npm publish. End users install with npm install -g local-project-console (then run local-project-console) or npx local-project-console (no install). There is also a desktop installer built from the desktop/ folder with npm install && npm run dist (NSIS .exe on Windows, .dmg on macOS, .AppImage on Linux) that needs no command line. Requirements: Node 18+; Ollama is optional and only enables AI mode. A common install failure is a native-module build error (re2/embeddings need Visual Studio C++ tooling via node-gyp on Windows) — those dependencies are optional, so the install succeeds without them and matching falls back to fuzzy/NLP stages; a sharp download timeout is fixed by simply retrying npm install; an EPERM/rmdir error means a locked folder (running process/antivirus/OneDrive) — close it, delete the partial AppData/Roaming/npm/node_modules/local-project-console, and retry from an elevated terminal. Never recommend --ignore-scripts to get past a build error — it silently breaks code search.`;
}
