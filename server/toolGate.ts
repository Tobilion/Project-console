...
type ToolPermission = 'ask' | 'deny' | 'allow-after-first-ask' | undefined;
type ToolArgs = { risky?: unknown; command?: unknown; importance?: unknown } | null | undefined;

/** Cache for plugin manifests to avoid reading the file on every tool creation. */
const pluginManifestCache = new Map<string, Promise<Record<string, any> | null>>();
...
export function invalidatePluginManifest(root: string): void {
  pluginManifestCache.delete(root);
}

export async function getPluginManifest(root: string): Promise<Record<string, any> | null> {
  ...
  const promise = loadPluginManifest(root)
    .then((result) => {
...
export const GATED_TOOLS: Set<string> = new Set(['writeFile', 'editFile', 'insertAtLine', 'appendToFile']);
 ...
 */
export const ALWAYS_CONFIRM_TOOLS: Set<string> = new Set(['runTests', 'stopProcess']);
 ...
 */
export const CUSTOM_RISKY_TOOLS: Map<string, Set<string>> = new Map();

export function isGatedToolCall(toolName: string, args?: ToolArgs): boolean {
  ...
  if (toolName === 'executeCommand' && (Boolean(args?.risky) || isDestructiveCommand(args?.command))) return true;
...
export function isCustomToolRisky(toolName: string, projectRoot: string): boolean {
...
export const ASK_MODE_BLOCKED_TOOLS: Set<string> = new Set([
...
export function isAskModeBlocked(toolName: string): boolean {
 ...
 */
export async function getToolPermission(projectRoot: string, toolName: string): Promise<ToolPermission> {
  if (!projectRoot || !toolName) return undefined;
  const manifest = await getPluginManifest(projectRoot);
  const permission = manifest?.permissions?.[toolName];
  return permission === 'deny' || permission === 'allow-after-first-ask' ? permission : undefined;
}
...
export function toolGrantKey(projectRoot: string, toolName: string): string {
 ...
 */
export async function resolveToolGate(toolName: string, args: ToolArgs, projectRoot: string | null | undefined, sessionGrants: Set<string> | null | undefined): Promise<GateDecision> {
  ...
  if (ALWAYS_CONFIRM_TOOLS.has(toolName) || (toolName === 'executeCommand' && (Boolean(args?.risky) || isDestructiveCommand(args?.command)))) {
