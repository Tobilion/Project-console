/**
 * Pre-semantic override pins (B.1 data split, 2026-09-09): the literal first-match rules
 * `findPreSemanticOverride()` scans before the embedding stage ever votes. Each pin covers
 * a phrase shape confirmed (via live user testing, not theory) to misclassify by pure
 * embedding similarity — e.g. "initialize git" landed on git_status instead of git_init.
 *
 * ORDER IS LOAD-BEARING: the matcher returns the FIRST matching pin, so several entries
 * document must-stay-first/before/after constraints inline — do not reorder without reading
 * those notes. Keep the list narrow (confirmed traps only, never a shadow matcher), and add
 * new pins as data entries here rather than inline code (supports the A-10 workflow).
 * Table-driven coverage for every pin lives in server/scripts/batteries/matcherBatteries.js
 * (run via `npm run check-matcher`), not in a second test layer next to this file.
 */
