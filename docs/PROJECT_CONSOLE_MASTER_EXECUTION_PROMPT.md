...
| A-15 | `server/matcherMulti.js` (`_splitConjunctions`), `server/wsHandlers/connectionMatching.js` | **[ADDED 2026-09-08 from live user report, NOT in the original 3 research passes]** Real report against the NetPulse project: "open the site and check network at 3" is a genuine compound request (serve the dev server, AND run a network-watch/port-check). Today the matcher resolves ONE winning intent (`run_project`/`npm_run`, correctly recognized and rendered as a confirm chip) and the second clause ("check network at 3") is silently dropped — no chip, no fallback, no acknowledgment it was even parsed. `_splitConjunctions` exists and can split on "and"/"then", but (a) it bails entirely the moment the input contains a quote character (no quote-boundary awareness — noted in `CLAUDE.md`'s matcher-traps section already), and (b) even when it does split, `matchMulti`'s whole-phrase guard (added in the 2026-08-26 conversational-quality pass to stop "commit and push" from wrongly splitting into two intents) can suppress legitimate multi-intent splits too aggressively — the two behaviors need to coexist: a single semantic intent phrase ("commit and push") must stay one card, while a genuine compound of two unrelated actions ("open the site AND check network at 3") must render as two. | Extend the multi-intent path so that when 2+ clauses each resolve to a *different* intent above its own confidence floor, EVERY recognized clause gets its own confirm/answer surface in the same turn — not just the highest-confidence one. Concretely: (1) fix `_splitConjunctions` to tolerate quoted substrings (split on top-level "and"/"then"/"," outside of quotes, not inside them); (2) when a split yields N clauses, run each independently through the matcher and render N chips/cards in the response (a confirm-gated card for each mutating clause, an inline answer for each read-only clause) rather than picking one winner and discarding the rest; (3) keep the existing whole-phrase guard for the single-intent-that-merely-contains-"and" case (e.g. "commit and push" is one `git_commit_push` intent, not two) by only splitting when the clauses resolve to genuinely different intent IDs at reasonable confidence — a split that resolves both halves to the same intent should collapse back to one card; (4) add a `MULTI-INTENT` battery to `matcherBatteries.js` covering: two different mutating intents ("open the site and check network at 3"), one intent that shouldn't split ("commit and push"), and a quoted-string edge case ("rename a.txt to \"my file.txt\" and open it") to lock in both directions of this fix. |
...
| D-9 | `server/state.js`, `server/scanCache.js`, `server/projectScanner.js`, `src/hooks/useConsoleTabs.ts`, `src/components/ProjectTabs.tsx` | **[ADDED 2026-09-08 from live user feedback, NOT in the original 3 research passes]** The console currently ties each browser TAB to exactly one scan root (`tabWorkspaces` Map, one `scanPath` per tab, see `CLAUDE.md`'s Phase T notes) — adding a second folder means opening a second tab, which is a reasonable model but not what was asked for: the user wants the ability to add MULTIPLE scan roots that all contribute to the SAME project list/workspace at once (e.g. one folder for client work, one for personal projects, browsed and searched together) without the console being conceptually "stuck to one path" per view. | Add a `scanRoots: string[]` concept alongside (not replacing) the existing per-tab single-root model: a tab can optionally track an array of roots instead of one, `discoverProjects` runs against each root and the results are merged into that tab's project list (de-duplicated by resolved absolute path, tagged with which root each project came from for the UI to group/filter by), and `POST /api/scan-path` gains an "add another folder to this tab" mode distinct from "replace this tab's folder" so the existing single-root behavior remains the default and this is additive. Cache/signature logic (`scanCache.js`) needs to key on the full root SET for a tab, not a single path, so the stale-while-revalidate work from D-2/D-3 still applies correctly when any one of several roots changes. UI: the scan-path input / folder picker needs an explicit "+ Add folder" affordance distinct from the existing "scan a new path" (replace) flow, and the project list/sidebar should be able to show which root a project came from when a tab has more than one. |
...
| F-11 | Chat-native rich tool UI (reminders, notes, and other tool results rendered inline) | **[ADDED 2026-09-08 from live user feedback, NOT in the original 3 research passes]** Today a tool result in chat is plain text (or a generic confirm card, per D-7's REST-migration work). The user wants specific tools to render as a genuinely tool-shaped interactive surface INSIDE the chat transcript, not a text block — concretely: calling up reminders in chat should produce an Apple-Reminders-style card (checkbox rows, tap/click to mark done, done items move to a struck-through section) rendered in place in the conversation, the same way a rich attachment renders in a modern chat UI, rather than routing the user out to the Reminders tool panel to do the same thing. The user asked that this same treatment be extended, tool by tool, to whichever other tools benefit from it (notes as a card with inline edit, backup list as a card with download/reveal buttons, PDF operation results as a card with the output file plus a preview thumbnail, etc.) rather than staying one-off to reminders. | This is a genuinely new UI primitive, not a bug fix — treat it as its own mini-project inside Phase F: (1) design one shared "inline tool card" component contract (a card type id, a data payload shape, and a small set of interactive affordances — checkbox/toggle, button, link) that chat messages can carry as an ADDITIVE field on the existing `answer`/WS message shape (same pattern already used for `openPanel` — never break CLI parity: the CLI must still get a sane plain-text rendering of the same data when it can't render the card); (2) build the first card type for reminders (list + inline complete/delete, wired to the same direct-REST endpoints from D-7 so marking one done in the chat card and in the Reminders panel are the same underlying write and stay in sync); (3) once that pattern is proven, extend it to at least one more tool (notes is the natural second candidate, given F-5/F-6 are already reworking its data model) so the framework is demonstrably reusable and not a one-off hack; (4) keep every card read-through-able as plain text for the CLI and for anyone with reduced motion / accessibility needs — the card is a progressive enhancement over a working plain-text answer, never a replacement that leaves the CLI with nothing. |
...
| K-10 | `server/doctor.js`, `bin/cli.js`'s `doctor` subcommand, Settings UI | **[ADDED 2026-09-08 at explicit user request, NOT in the original 3 research passes]** `console doctor` (added 2026-08-26) already runs a solid set of read-only checks (ports free, daemon alive, embedding model cached, data dir writable, Ollama reachable, npm update status, tooling on PATH, disk free) but is diagnose-only — it tells you a port is stuck or a cache dir is stale, then leaves you to fix it by hand, and it's CLI/chat-only with no equivalent inside the web/desktop Settings UI where most users will actually be when something's wrong. | Upgrade the doctor in three directions, all additive to the existing checks (do not remove or weaken any of them): (1) **auto-fix mode** — add a `--fix` flag to `node bin/cli.js doctor` (and a matching "Fix" button per-check in a Settings "Diagnostics" panel) that safely remediates the specific failures doctor already knows how to detect and where a fix is unambiguous and non-destructive: clear a stale `logs/daemon.port`/`daemon.pid` when no process actually owns that port (reuses the same command-line-verification kill-safety logic already in `scripts/daemon.mjs`, never blind-deletes), sweep orphaned `*.tmp` files (the exact sweep A-6 already calls for — implement A-6 as doctor's first auto-fix rather than a separate one-off boot-time sweep, so the same logic serves both a background hygiene pass AND an on-demand user-triggered fix), and re-download/re-verify a corrupted embedding-model cache directory. Each auto-fix must report exactly what it changed (never a silent fix) and must be safe to run repeatedly with zero effect when there's nothing to fix. (2) **JSON output mode** — `doctor --json` for scriptability/automation and so a future Settings panel can render structured results instead of scraping console text. (3) **reachable from the UI, not just the CLI** — a "Diagnostics" section in Settings (or its own panel, following the existing Tool Panel registry pattern in `server/toolPanelRegistry.js`) that runs the same checks via a new `GET /api/doctor` route and renders each check's status with an inline "Fix" button where an auto-fix exists, so a non-technical user hitting a boot problem never needs to open a terminal at all. Keep doctor's existing hard constraint intact: it must still run and be useful even when the main server won't boot, so the new REST route is additive (served once the server IS up) — the CLI/standalone path remains the one that works when it isn't. |
| K-11 | New: an in-app troubleshooting/self-repair flow, distinct from the doctor CLI tool | **[ADDED 2026-09-08 at explicit user request, NOT in the original 3 research passes]** Beyond the doctor upgrade above (which is a proactive, on-demand health check), the user wants errors the app actually HITS during normal use to be easy to troubleshoot and fix in the moment — today a fatal boot error shows the existing crash/error screen (`desktop/main.cjs`'s `showFatalError`, `serverStderrTail`) with a raw log tail, and a failed tool call in chat just shows an error bubble, both of which dead-end a non-technical user. | Add a "Troubleshoot this" affordance wherever the app currently shows a raw error to the user: (1) on the existing fatal-error screen (`desktop/main.cjs`), add a "Troubleshoot" button next to the existing Retry/Quit choices that runs the relevant subset of doctor's checks (ports/daemon/cache/Ollama, whichever ones plausibly relate to the failure that was captured in `serverStderrTail`) and, where doctor has a matching auto-fix (K-10), offers to apply it before retrying — so a stuck port or a corrupted cache becomes a one-click fix instead of a dead end; (2) in chat, when a tool call fails (the existing `tool_result` error path) or a command hits a genuine error (not a user cancellation), append a short, specific "what likely went wrong" line plus, where applicable, a chip offering the relevant fix (re-run with sandbox off, check if Ollama is running, re-scan the project) rather than a bare stack-trace-shaped error string — this directly extends the K-5 error-message-specificity fix into an actionable next step, not just a clearer description of the problem; (3) log every troubleshoot-flow invocation (which check failed, whether an auto-fix was offered/accepted, whether the retry then succeeded) so future doctor/troubleshoot coverage gaps are visible from real usage data rather than guesswork. This item and K-10 should land together — K-11 is the "surface it to the user at the moment of failure" half of the same effort K-10 makes possible on demand. |
