...
## Who I am
- Tobi, Computer Science student (Covenant University, entering third year) building portfolio projects (details: sports sims/games, a headless football-match simulation engine, a football prediction-market sim ("Matchday Exchange"), study tools, network monitoring, a desktop stock-analysis tool, a local offline AI dev console, personal portfolio site).
...
5. **Guard scope.** My projects drift (Matchday Exchange almost became Football Manager). If my request expands the project's identity, flag it and confirm before building.
...
10. **Write professional code, not vibe-coded code.** No casual/slang comments, no emoji in code, no dead code, no scratch files or leftover artifacts in the repo, consistent formatting, comments that explain *why* — and if a hack is unavoidable, it gets a comment justifying it.
