...
Verified: check-handlers 287/287, check-ws-cases clean, npm test 602/602, tsc clean).
  ...
  — deliberately left out of tuningStore, which stays backend-numbers-with-bounds-only). The
   ...
   outstanding (no browser here). B.3's `COMMON_DEV_PORTS` row is also done (2026-09-09,
   opencode): no duplicated list existed anywhere (only comments/examples mention those
...
- **Phase E: started** (commits e3d46ed, e0bc8ed, 2026-09-09 — see below). E-1 done
  ...
  interval for the same `/api/notifications` data. tsc clean. E-6 done per user scoping
  decision (2026-09-09, opencode — additive, not replacement: bubbles stay as the CLI's
  only signal, web additionally toasts): routine confirmations now carry additive
  `toast: true` — notes.create + reminders.create + schedule create/remove-success +
...
- **Phase C: started** (2026-09-09, opencode). C-1 verified already covered — no change
  ...
  performing the action is deliberately deferred (the spec's own "don't force this
...
- `server/executor.js` — ~359-line orchestrator (`executeCommand` + venv rewrite; refuses only
  ...
  not inline literals.
- `server/matcher.js` — ~312-line orchestrator; leaves: `intentRegistry.js` (`BUILTIN_INTENTS`
...
- **CI**: .github/workflows/desktop-build.yml builds .dmg (macos-latest) and .AppImage
  ...
  desktop/dist/Project Console Setup 1.0.0.exe (clean-tested 2026-08-25 (see 02-findings.md P0 — the round-5 boot test ran against dev node_modules and never caught the vite static-import crash; the installed-app test now covers it)
...
# Audit round 6 (2026-08-25) — desktop packaging P0s, fresh-install data, auto-update, self-docs

Full detail: 02-findings.md. Highlights:

- **P0: packaged app crashed at boot — vite static import**. server/index.js statically imported vite (devDependency); the staged prod node_modules never contains it (verified: `npm ci --omit=dev` skips packages listed in BOTH sections — the round-5 "clean test" ran against dev node_modules and never caught this). Fixed: dynamic `await import('vite')` inside the non-production branch only; vite removed from `dependencies` (kept in devDependencies). Same masking class found + fixed in `server/pdfKit.js`: pdf-parse → pdfjs-dist legacy evaluates `new DOMMatrix()` at module scope and crashed the whole server whenever the `@napi-rs/canvas` native binding is missing (reproduced via `--omit=optional`) — now a lazy guarded import; text extraction degrades, PDF build/split/watermark keep working.
- **Fresh-install data**: stage-server.mjs stages an EMPTY data/ (was copying the developer's live profile/conversations/telemetry/clipboard + pre-enabled settings). readProfile() defaults setupComplete:false → first-run onboarding shows. Verified on installed builds.
